...
class Github_Updater {
	...
	/**
	 * Plugin basename (e.g. "fancy-product-page/fancy-product-page.php").
	 *
	 * @var string
	 */
	...
	public function __construct() {
		$this->plugin_basename = \PP_FPP_BASENAME;
	...
	private function is_enabled(): bool {
		/**
		 * Filter whether GitHub auto-updates are enabled for Fancy Product Page.
		 *
		 ...
		 */
		return (bool) apply_filters( 'fancy_product_page_updater_enabled', true );
	}
	...
	public function check_for_update( $transient ) {
		...
		if ( empty( $checked ) ) {
			...
			} elseif ( version_compare( \PP_FPP_VERSION, $release['version'], '>=' ) ) {
				$this->log( 'check_for_update: current version ' . \PP_FPP_VERSION . ' is up to date (latest: ' . $release['version'] . ').' );
			} else {
				$this->log( 'check_for_update: update available ' . \PP_FPP_VERSION . ' → ' . $release['version'] . '.' );
	...
	public function plugin_info( $result, $action, $args ) {
		...
		if ( is_array( $release ) ) {
			...
			$plugin_data = get_plugin_data( \PP_FPP_FILE, false, true );
	 ...
	 * (e.g. "fancy-product-page.zip" or "fancy-product-page-1.2.0.zip").
	...
	private function log( string $message ): void {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'Fancy_Product_Page Github_Updater: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional debug logging.
		}
	}
	...
	private function log_error( string $message ): void {
		error_log( 'Fancy_Product_Page Github_Updater [error]: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional error logging for updater failures.
	}
}
